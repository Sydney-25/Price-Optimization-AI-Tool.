...
data = {
    ...
    'Sales_Amount_After_Discount': [11272.59, 11272.59, 11272.59, 11272.59, 11272.59, 12000, 12000, 12000, 12000, 12000, 12500, 12500, 12500, 12500, 12500, 13000, 13000, 13000, 13000, 13000] #Example sales
}
...
default_competitor_price = float(df['Competitor_Price'].mean()) #use mean of existing values as a default value
competitor_price = st.number_input("Competitor's Price", value=default_competitor_price, min_value=0.0)
...
st.sidebar.write("This app uses a Linear Regression model to predict sales based on your product's price, competitor's price, and item quantity. The model is trained on a small, embedded dataset. Provide the necessary inputs to find the price that maximizes your predicted sales.")
st.sidebar.write("Note: This is a simplified example. For real-world scenarios, you would need a much larger and more representative dataset, and consider more sophisticated modeling techniques.")
