...
def train_model(df):
    ...
    # Feature Engineering
    # Ensure 'Fiscal_Week_ID' is in 'YYYY-MM' format (pad single-digit months)
    df['Fiscal_Week_ID'] = df['Fiscal_Week_ID'].apply(
        lambda x: f"{x.split('-')[0]}-{x.split('-')[1].zfill(2)}"
    )
    df['Fiscal_Week_ID'] = pd.to_datetime(df['Fiscal_Week_ID'], format='%Y-%m').astype('int64')  # Convert to timestamp
...
def load_data():
    ...
    if uploaded_file is not None:
        try:
            ...
            expected_columns = ['Fiscal_Week_ID', 'Store_ID', 'Item_ID', 'Price', 'Item_Quantity', 'Sales_Amount_No_Discount', 'Sales_Amount', 'Competition_Price']
